...
class Controller_CaseNote extends Controller_CController
{
	protected $model;
	public function __construct($request)
	{
		parent::__construct($request);
		$this->model=Model::factory('casenotemd');
	...
	public function action_list()
	{
		...
		try
		{
			...
			$paging=Pagination::factory(array('base_url'=> "casenote/list/",'uri_segment' => 'page','total_items'=> $total, 'items_per_page' => $volume, 'style'=> 'digg'));
			$view->paging=$paging;
		}
		catch(Exception $e)
		{
	...
	public function action_index()
	{
		...
		try
		{
			...
			$val = Validate::factory($_POST)
				->filter(TRUE, 'trim')
				->filter('id', 'trim')
				->filter('patientid', 'trim')
				->filter('date', 'trim')
				->filter('doctorid', 'trim')
				->filter('note', 'trim')
			...
			if(isset($_POST['submit'])):
				$post['patientid']=Html::chars($_POST['patientid']);
				$post['date']=Html::chars($_POST['date']);
				$post['doctorid']=Html::chars($_POST['doctorid']);
				$post['note']=Html::chars($_POST['note']);
				$post['attachment']=Html::chars($_POST['attachment']);
			...
			elseif(isset($_POST['submit']))
			{
				...
				$this->request->redirect("casenote/index");
			}
		}
		catch(Exception $e)
		{
